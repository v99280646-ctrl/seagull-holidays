...
export function DestinationCard({ dest }: { dest: Destination }) {
  return (
    ...
    >
      ...
      <span className="absolute left-3 top-3 rounded-full bg-background/90 px-2.5 py-1 text-[11px] font-semibold text-primary sm:left-4 sm:top-4 sm:px-3 sm:text-xs">
        {dest.tag}
      </span>
      <div className="absolute inset-x-0 bottom-0 p-4 text-primary-foreground sm:p-5">
        <p className="flex items-center gap-1 text-[11px] text-primary-foreground/80 sm:text-xs">
          <MapPin className="h-3.5 w-3.5" />{dest.region}
        </p>
        <h3 className="mt-1 text-[17px] font-bold leading-tight sm:text-xl">{dest.name}</h3>
        <p className="mt-1 text-xs text-primary-foreground/85 sm:text-sm">{dest.packages} packages available</p>
      </div>
    </Link>
  );
}
